...
Windows Electron 与 iPhone Safari / PWA 共用这次前端和 Express 改进。Android 使用同一套新版界面，但原生 Java 音源调度不在本次后端改动范围内。

- **无损优先调度**：页面请求真正进入多音源无损择优流程；每个源返回后立即探测，首个验证为无损的候选可以直接返回，不必等待其他慢源。没有已验证无损时，选择已验证的可播放候选，再考虑未验证地址。
- **真实格式识别**：探测只读取最多 4 KiB，服务器忽略 Range 时也主动取消剩余流。拒绝错误页面、403 和空响应；不把扩展名或宣称的码率当作无损证据。区分 AAC / MP3，WAVE 只在识别到 PCM / 浮点格式时标记无损。MP4 容器不能单凭名字认定为 ALAC。
- **链接恢复**：前端直链缓存缩短为 45 秒，重复请求合并；恢复请求显式绕过服务端缓存。上游失败不再返回过期直链。缓冲先刷新无损，再尝试 HQ，每首最多恢复三次；切歌后的旧请求不能覆盖新歌。
- **搜索与性能**：只采纳最新搜索结果，离开搜索页后不被迟到响应拉回；歌词定位采用二分查找；界面减少大面积模糊合成；音质偏好保存在本机。
- **新版界面**：参考网易云的信息布局，采用红色导航、深色音乐库、唱片主题首页、彩色榜单和固定底部播放器。手机端保留底部导航、迷你播放器及安全区。
- **PWA 更新**：同步页面与 Service Worker 的资源版本及主题色，保留现有安装身份。

无损依赖上游提供真实无损文件和设备能够解码；这些改进不能创造上游不存在的无损音源，也不保证所有第三方接口持续可用。这里的“验证”指检测文件格式，不是鉴定录音母带或证明原始文件未由有损音频转码。未知音质明确显示“音质未验证”。iPhone 已下载 FLAC 的原有服务端 ALAC 转换路径继续保留，仍需要部署环境具备 FFmpeg；在线流与锁屏播放需真机验收。
...
npm test
npm run test:pwa
node --test tests/playback-reliability.test.js tests/music-api.test.js
```

发布前验收：

1. Windows 运行 `npm run dist` 和 `npm run qa:electron`，检查搜索、连续切歌、收藏和播放队列。
2. 在 HTTPS iPhone Safari 与主屏幕 PWA 中检查窄屏布局、横竖屏、安全区、弹窗滚动和更新提示。
3. 使用自己可访问的音源，分别播放 FLAC、MP3、ALAC，检查实际音质标记；测试失效直链、弱网、暂停后恢复以及连续快速切歌。
4. iPhone 锁屏持续播放、自动下一首和中断后恢复需要真机测试，浏览器尺寸模拟不能替代。

### 可复现的视觉测试

使用 Node 20.19+ 或 22.12+，执行 `npm run preview:ui`，打开终端显示的地址。`/__qa?width=390&height=844` 可检查指定尺寸；`npm run dev` 仍启动 Electron。预览使用真实前端、固定测试曲目和合成静音 WAV，不连接真实账号及在线音源。搜索 `empty` 可查看空结果。

本次浏览器检查覆盖 1363×936、800×600、390×844 和 320×568，检查首页、搜索长标题、播放器、队列和登录弹窗。修复了手机迷你播放器文字拥挤、320px 登录关闭按钮重叠、首页标题断行、紧凑桌面导航文字隐藏，以及界面重绘丢失已验证音质标签。真机 Safari 安全区、锁屏播放和 Windows 安装包仍需独立验收。
